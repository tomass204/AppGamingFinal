...
class DebateViewModel(
    ...
    private val _uiState = mutableStateOf(DebateUiState())
    val uiState: State<DebateUiState> = _uiState
    ...
    fun loadDebates() {
        viewModelScope.launch {
            ...
            val debates = publicaciones.filter {
                it.tipo.equals(PublicacionCategory.DEBATE.name, ignoreCase = true)
            }.map { debate ->
                val likeId = likesMap[debate.publicacionID]
                val favoritoId = favoritosMap[debate.publicacionID]

                debate.copy(
                    isLiked = likesMap.containsKey(debate.publicacionID),
                    isFavorited = favoritosMap.containsKey(debate.publicacionID),
                    likeID = likeId,
    ...
    fun toggleLike(publicacion: Publicacion) {
        viewModelScope.launch {
            ...
            val actuales = _uiState.value.debates.toMutableList()
            ...
            _uiState.value = _uiState.value.copy(debates = actuales)
